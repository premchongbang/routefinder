...
app.post('/login_check', function(req, res, next){
  pg.connect(connectionString, function(err, client, done){
    ...
    if(user_name == "" || pw == ""){
      ...
      async.waterfall([
        function(callback){
          var query = "SELECT * FROM student WHERE username = $1 AND password = $2";
          client.query(query, [user_name, pw], function(err, result){
            if(err) {
...
app.post('/add_event', function(req, res, next) {
  pg.connect(connectionString, function(err, client, done){
    ...
    if(user_name == "undefined" || society_name == "" || event_name == "" || date == "" || description == "" || !helper.checkDate(date)){
      ...
      async.waterfall([
        function(callback){
          var query = "INSERT INTO public.event(organiser, event_title, date, detail, latlng, student_username, venue) VALUES ($1, $2, $3, $4, $5, $6, $7)";
          
          client.query(query, [society_name, event_name, date, description, position, user_name, venue], function(err){
            if(err){ 
              console.log(err);
              callback(err, "db");
            } else {
...
app.post('/edit_event', function(req, res, next) {
  pg.connect(connectionString, function(err, client, done){
    ...
    if(user_name == ""){
      ...
      async.waterfall([
        function(callback){
          var query = "SELECT * FROM event WHERE student_username = $1";
          client.query(query, [user_name], function(err, result){
            if(err) {
              return callback(err, "db");
            } else {
  ...
  pg.connect(connectionString, function(err, client, done){
    ...
    if(btn == "Update"){
      if(user_name == "" || society_name == "" || event_title == "" || date == "" || description == "" || !helper.checkDate(date)){
        async.waterfall([
          function(callback){
            var query = "SELECT * FROM event WHERE  student_username = $1 ORDER BY event_id ASC;";
            client.query(query, [user_name], function(err, result){
              if(err) {
                return callback(err, "db");
              } else {
        ...
        async.waterfall([
          function(callback){
            var query = "UPDATE public.event SET organiser = $1, event_title= $2, date= $3, detail= $4, venue= $5 WHERE event_id= $6;";
            client.query(query, [society_name, event_title, date, description, venue, event_id], function(err){
              if(err){
                console.log(err);
                return callback(err, "db");
              } else {
      ...
      async.waterfall([
        function(callback){
          var query = "DELETE FROM event WHERE event_id = $1;";
          client.query(query, [event_id], function(err){
            if(err){ 
              console.log(err);
               return callback(err, "db");
            } else {
